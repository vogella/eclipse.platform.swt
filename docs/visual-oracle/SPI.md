...
Everything in the harness builds against these shapes, so they change rarely and deliberately.
...
| `org.eclipse.swt.visualoracle.impl` | Implementations behind the SPI. Nothing outside the harness may rely on them. |
...
| `run` | implemented | Runs a selection of specimens across two backends and compares them. |
| `triage` | implemented | Ranks the differences of a previous run. |

Exit codes: 0 success, 1 selftest failure or runtime error, 2 usage error.

## Deviations from the PLAN.md SPI sketch

Where the sketch and this document disagree, this document wins.
